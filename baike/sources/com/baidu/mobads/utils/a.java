...
public class a implements IBase64 {
    ...
    public String encode(String str) {
        ...
        while (length < bytes.length) {
            ...
            bArr[i + 2] = a[((bytes[length + 1] & 15) << 2) + ((bytes[length + 2] & JfifUtil.MARKER_SOFn) >> 6)];
    ...
    private byte[] a(String str) {
        ...
        if (!(b(str) || str == null || str.length() < 4)) {
            byte b;
            byte b2;
            if (str.charAt(str.length() - 2) == Typography.dollar) {
                bArr = new byte[((((str.length() / 4) - 1) * 3) + 1)];
            } else if (str.charAt(str.length() - 1) == Typography.dollar) {
