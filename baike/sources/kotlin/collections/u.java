...
class u extends t {
    private static final int a(@NotNull List<?> list, int i) {
        int i2 = (i < 0 || i > q.getLastIndex(list)) ? 0 : 1;
        if (i2 != 0) {
            return q.getLastIndex(list) - i;
        }
        throw new IndexOutOfBoundsException("Element index " + i + " must be in range [" + new IntRange(0, q.getLastIndex(list)) + "].");
    }

    private static final int b(@NotNull List<?> list, int i) {
        int i2 = (i < 0 || i > list.size()) ? 0 : 1;
        if (i2 != 0) {
            return list.size() - i;
        }
