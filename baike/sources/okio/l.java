...
final class l implements BufferedSource {
    ...
    public long read(Buffer buffer, long j) throws IOException {
        ...
        } else if (this.buffer.b == 0 && this.source.read(this.buffer, PlaybackStateCompat.ACTION_PLAY_FROM_URI) == -1) {
            return -1;
        } else {
    ...
    public int select(Options options) throws IOException {
        ...
        do {
            int a = this.buffer.a(options);
            if (a == -1) {
                return -1;
            }
            int size = options.a[a].size();
            if (((long) size) <= this.buffer.b) {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.buffer.b == 0 && this.source.read(this.buffer, PlaybackStateCompat.ACTION_PLAY_FROM_URI) == -1) {
            return -1;
        }
        return this.buffer.read(bArr, i, (int) Math.min((long) i2, this.buffer.b));
    }
    ...
    public String readUtf8Line() throws IOException {
        ...
        if (indexOf == -1) {
            return this.buffer.b != 0 ? readUtf8(this.buffer.b) : null;
        } else {
