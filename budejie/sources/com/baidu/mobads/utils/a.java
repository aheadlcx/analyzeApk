...
public class a implements IBase64 {
    ...
    public String encode(String str) {
        ...
        while (length < bytes.length) {
            bArr[i] = a[(bytes[length] & R$styleable.Theme_Custom_theme_bg_color) >> 2];
            bArr[i + 1] = a[((bytes[length] & 3) << 4) + ((bytes[length + 1] & R$styleable.Theme_Custom_shape_cmt_like4_bg) >> 4)];
