...
class IncreasingQualityDataSourceSupplier$IncreasingQualityDataSource extends AbstractDataSource<T> {
    ...
    public IncreasingQualityDataSourceSupplier$IncreasingQualityDataSource(IncreasingQualityDataSourceSupplier increasingQualityDataSourceSupplier) {
        ...
        int size = IncreasingQualityDataSourceSupplier.access$000(increasingQualityDataSourceSupplier).size();
        this.mNumberOfDataSources = size;
        this.mIndexOfDataSourceWithResult = size;
        this.mDataSources = new ArrayList(size);
        int i = 0;
        while (i < size) {
            DataSource dataSource = (DataSource) ((Supplier) IncreasingQualityDataSourceSupplier.access$000(increasingQualityDataSourceSupplier).get(i)).get();
            this.mDataSources.add(dataSource);
            dataSource.subscribe(new IncreasingQualityDataSourceSupplier$IncreasingQualityDataSource$InternalDataSubscriber(this, i), CallerThreadExecutor.getInstance());
