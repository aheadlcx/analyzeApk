...
public class a {
    ...
    public static AdError a(int i) {
        switch (i) {
            ...
            case 400:
            case 403:
            case ErrorCode.SERVER_REQUESTDATAMISSED /*3001*/:
                return new AdError(ErrorCode.SERVER_REQUESTDATAMISSED, "网络异常");
            case 404:
            case 4011:
            ...
            case ReaderCallback.READER_PDF_LIST /*5008*/:
                return new AdError(ReaderCallback.READER_PDF_LIST, "图片加载错误");
            case 500:
            case ErrorCode.SERVER_CREATEPROXYERROR /*4003*/:
                return new AdError(ErrorCode.SERVER_CREATEPROXYERROR, "广告位错误");
            case 501:
            case ReaderCallback.SEARCH_SELECT_TEXT /*5004*/:
                return new AdError(ReaderCallback.SEARCH_SELECT_TEXT, "没有广告");
            case 502:
            case ReaderCallback.HIDDEN_BAR /*5001*/:
                return new AdError(ReaderCallback.HIDDEN_BAR, "服务端数据错误");
            case 600:
            case 4004:
            ...
            case 602:
            case ErrorCode.SERVER_REMOTEACCESSEXCEPTION /*4002*/:
                return new AdError(ErrorCode.SERVER_REMOTEACCESSEXCEPTION, "Manifest文件中Activity/Service/Permission的声明有问题");
            case 603:
            case 200201:
            ...
            case ReaderCallback.SHOW_BAR /*5002*/:
                return new AdError(ReaderCallback.SHOW_BAR, "视频素材下载错误");
            case 701:
            case ReaderCallback.COPY_SELECT_TEXT /*5003*/:
                return new AdError(ReaderCallback.COPY_SELECT_TEXT, "视频素材播放错误");
            case 800:
                return new AdError(4012, "内容接口调用顺序错误，调用点击接口前未调用曝光接口");
            case 2002:
                return new AdError(2002, "内部错误");
            case ErrorCode.SERVER_REQUESTTIMEOUT /*4001*/:
                return new AdError(ErrorCode.SERVER_REQUESTTIMEOUT, "传入的参数有错误");
            case 4008:
                return new AdError(4008, "设备方向不适合展示广告");
            case ReaderCallback.READER_TOAST /*5005*/:
            case 109506:
                return new AdError(ReaderCallback.READER_TOAST, "广告请求量或者消耗等超过日限额，请明天再请求广告");
            case ReaderCallback.SHOW_DIALOG /*5006*/:
            case 107030:
                return new AdError(ReaderCallback.SHOW_DIALOG, "包名校验错误，当前App的包名和广点通移动联盟官网注册的媒体包名不一致，因此无广告返回");
            case ReaderCallback.READER_PPT_PLAY_MODEL /*5009*/:
            case 109507:
                return new AdError(ReaderCallback.READER_PPT_PLAY_MODEL, "广告请求量或者消耗等超过小时限额，请一小时后再请求广告");
            case 107034:
                return new AdError(ReaderCallback.READER_TXT_READING_MODEL, "广告样式校验失败，请检查广告位与接口使用是否一致");
            case 107035:
                return new AdError(4013, "使用支持视频素材的原生模板广告位前，请升级您的SDK");
            case 400101:
                return new AdError(ErrorCode.SERVER_REQUESTTIMEOUT, "传入的参数有错误，详细码：" + i);
            case 400102:
                return new AdError(ErrorCode.SERVER_REQUESTTIMEOUT, "传入的参数有错误，详细码：" + i);
            case 400103:
                return new AdError(ErrorCode.SERVER_REQUESTTIMEOUT, "传入的参数有错误，详细码：" + i);
            case 400104:
                return new AdError(ErrorCode.SERVER_REQUESTTIMEOUT, "传入的参数有错误，详细码：" + i);
            default:
                return new AdError(ErrorCode.SERVER_SERVICENOTFOUND, "未知错误，详细码：" + i);
        }
    }
    ...
    public static JSONObject a(DeviceStatus deviceStatus) throws JSONException {
        ...
        if (deviceStatus != null) {
            ...
            jSONObject.putOpt(x.ae, deviceStatus.getLat());
            jSONObject.putOpt(x.af, deviceStatus.getLng());
    ...
    private static boolean b(Context context) {
        String[] strArr = new String[]{UpdateConfig.h, UpdateConfig.g, "android.permission.ACCESS_WIFI_STATE", "android.permission.READ_PHONE_STATE", UpdateConfig.f};
        int i = 0;
        while (i < 5) {
            try {
