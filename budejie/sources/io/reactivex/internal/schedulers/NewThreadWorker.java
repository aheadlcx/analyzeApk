...
public class NewThreadWorker extends Scheduler$Worker implements Disposable {
    ...
    public Disposable scheduleDirect(Runnable runnable, long j, TimeUnit timeUnit) {
        Future submit;
        Object scheduledDirectTask = new ScheduledDirectTask(RxJavaPlugins.onSchedule(runnable));
        if (j <= 0) {
            try {
    ...
    public ScheduledRunnable scheduleActual(Runnable runnable, long j, @NonNull TimeUnit timeUnit, @Nullable DisposableContainer disposableContainer) {
        Object scheduledRunnable = new ScheduledRunnable(RxJavaPlugins.onSchedule(runnable), disposableContainer);
